...
export function Workspace() {
  const [url, setUrl] = useState<string | null>(null), [rawText, setRawText] = useState(""), [text, setText] = useState(""), [summary, setSummary] = useState("");
  const [view, setView] = useState<"cleaned" | "raw" | "summary">("cleaned");
  ...
  async function load(file: File) {
    ...
    try {
      ...
      setText(extracted.cleanedText);
      setSummary("");
      setView("cleaned");
      setStatus(extracted.cleanedText.length >= 50 ? "อ่านและจัดเรียงข้อความแล้ว" : "ไม่พบข้อความที่เพียงพอใน PDF");
    }
  ...
  return <main className="mx-auto min-h-screen max-w-[1600px] p-4 md:p-6">
    ...
    <div className="grid gap-5 lg:grid-cols-2"><PdfViewer url={url} /><section className="flex min-h-[560px] flex-col rounded-2xl border bg-white p-4 shadow-sm">
      ...
      {rawText && <div className="mb-3 flex gap-1 rounded-xl bg-slate-100 p-1 text-xs"><button onClick={() => setView("cleaned")} className={`rounded-lg px-3 py-1.5 ${view === "cleaned" ? "bg-white shadow-sm" : ""}`}>ข้อความจัดเรียงแล้ว</button><button onClick={() => setView("raw")} className={`rounded-lg px-3 py-1.5 ${view === "raw" ? "bg-white shadow-sm" : ""}`}>ข้อความต้นฉบับ</button>{summary && <button onClick={() => setView("summary")} className={`rounded-lg px-3 py-1.5 ${view === "summary" ? "bg-white shadow-sm" : ""}`}>ผลสรุป</button>}</div>}
      <textarea aria-label="Extracted PDF text" readOnly={view === "raw"} className="min-h-52 flex-1 resize-none rounded-xl border p-3 text-sm outline-none focus:border-cyan-600" value={view === "summary" ? summary : view === "raw" ? rawText : text} onChange={(e) => view === "summary" ? setSummary(e.target.value) : setText(e.target.value)} placeholder="ข้อความจาก PDF และผลสรุปจะแสดงที่นี่" />
      <div className="mt-3 flex gap-2"><button disabled={text.length < 50} onClick={summarize} className="rounded-xl bg-slate-900 px-4 py-2 text-sm text-white disabled:opacity-40">สรุปเอกสาร</button><button onClick={() => navigator.clipboard.writeText(view === "summary" ? summary : view === "raw" ? rawText : text)} className="rounded-xl border px-4 py-2 text-sm">คัดลอก</button>{url && <button onClick={() => { setUrl(null); setRawText(""); setText(""); setSummary(""); setView("cleaned"); }} className="ml-auto rounded-xl border px-4 py-2 text-sm">เปลี่ยนไฟล์</button>}</div>
    </section></div>
  </main>;
}
