...
import { buildGeminiPrompt, buildMergePrompt, buildPrompt, CLOUD_SYSTEM_PROMPT } from "./prompts";
...
async function summarizeWithGroq(input: SummarizeInput, signal?: AbortSignal): Promise<string> {
  ...
  for (const chunk of chunkText(input.text)) {
    const response = await client.chat.completions.create({ model: GROQ_MODEL, temperature: 0.1, reasoning_effort: "none", reasoning_format: "hidden", messages: [{ role: "system", content: CLOUD_SYSTEM_PROMPT }, { role: "user", content: buildPrompt(input.mode, chunk) }] }, { signal });
    const content = sanitizeModelOutput(response.choices[0]?.message.content ?? "");
    if (!content) throw new Error("Groq returned an empty response");
    partials.push(content);
  }
  if (partials.length === 1) return partials[0];
  const merged = await client.chat.completions.create({ model: GROQ_MODEL, temperature: 0.05, reasoning_effort: "none", reasoning_format: "hidden", messages: [{ role: "system", content: CLOUD_SYSTEM_PROMPT }, { role: "user", content: buildMergePrompt(input.mode, partials.join("\n\n---\n\n")) }] }, { signal });
...
export async function summarizeDocument(input: SummarizeInput, signal?: AbortSignal): Promise<CloudSummary> {
  try { return { summary: await summarizeWithGroq(input, signal), provider: "Groq" }; }
    ...
    return { summary: await generateWithGemini(buildGeminiPrompt(input.mode, input.text), signal), provider: "Gemini" };
  }
}
