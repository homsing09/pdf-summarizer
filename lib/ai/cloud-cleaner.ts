...
export async function repairTextInCloud(text: string, mode: "auto" | "all", signal?: AbortSignal): Promise<string> {
  ...
  for (let index = 0; index < chunks.length; index += 20) {
    const response = await fetch("/api/repair-text", {
      ...
      body: JSON.stringify({ chunks: chunks.slice(index, index + 20) }),
      signal,
    });
